...
def plot_fits(file_list, logSpace, radiusScale, planet=60, lBoundary=15, uBoundary=90):
    """
    INPUTS:
    --------
    ...
    """
    planet_locale = round(planet * 3/4)
    ...
    for i in range(len(file_list)):
        ...
        plt.plot(length[lBoundary:uBoundary] * r_scale, intensity_profile, label=filename)
...
def kanagawa(file_list, png_name ='test.png', planet=60, search_radius=0.5, logSpace=False, radiusScale=False):
    ...
    ### This is the AU per pixel
    scale = 3/4
    
    ### Planet locale in pixels, rounded to a pixel/bin
    planet_locale = round(planet * 1/scale)
